  ...
  it(`CTrip_Order - 3`, async () => {
    ...
    for (const item of feeds) {
      ...
      for (const passenger of extrasData.PassengerInfoList) {
        if (passenger.OrderTicket) {
          if (passenger.OrderTicket.length !== extrasData.TicketInfoList.length) {
            console.info(item.orderId, extrasData.BasicInfo.NewOrderStatusName)
          }
        }
      }
    }
